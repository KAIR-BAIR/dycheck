...
for SEQUENCE in $SEQUENCES; do
    ...
    for MODEL in ${MODELS[*]}; do
        python tools/launch.py \
            --gin_configs "configs/${DATASET}/${MODEL}/base.gin" \
            --gin_bindings "Config.engine_cls=@Evaluator" \
            --gin_bindings "Tasks.task_classes=(@Video,)" \
            --gin_bindings 'SEQUENCE="'"${SEQUENCE}"'"'
        sleep 5s
        python tools/launch.py \
            --gin_configs "configs/${DATASET}/${MODEL}/randbkgd.gin" \
            --gin_bindings "Config.engine_cls=@Evaluator" \
            --gin_bindings "Tasks.task_classes=(@Video,)" \
            --gin_bindings 'SEQUENCE="'"${SEQUENCE}"'"'
        sleep 5s
        python tools/launch.py \
            --gin_configs "configs/${DATASET}/${MODEL}/randbkgd_depth.gin" \
            --gin_bindings "Config.engine_cls=@Evaluator" \
            --gin_bindings "Tasks.task_classes=(@Video,)" \
            --gin_bindings 'SEQUENCE="'"${SEQUENCE}"'"'
        sleep 5s
